...
describe("Set State", () => {
    ...
    test("Should reject with an error for an invalid state", async () => {
        ...
        await expect(setState(interfaceName, invalidState)).rejects.toThrowError(
            /Invalid state/,
        );
    });
    ...
    test("Should reject with an error for a non-existent interface", async () => {
        ...
        const state = "up";
        await expect(setState(nonExistentInterface, state)).rejects.toThrowError(
            /Cannot find device/,
        );
    });
});
...
describe("setMTU", () => {
    ...
    test("Should reject with an error for an invalid MTU value", async () => {
        ...
        await expect(setMTU(interfaceName, invalidMTU)).rejects.toThrowError(
            /Invalid MTU value/,
        );
    });
    ...
    test("Should reject with an error for a non-existent interface", async () => {
        const nonExistentInterface = "nonexistent";
        const mtuValue = 1500;
        await expect(setMTU(nonExistentInterface, mtuValue)).rejects.toThrowError(
            /Cannot find device/,
        );
    });
});
...
describe("Rename", () => {
    test("should rename a network interface", async () => {
        ...
        await expect(
            rename(oldInterfaceName, newInterfaceName),
    ...
    test("should reject with an error for invalid interface names", async () => {
        ...
        const newInterfaceName = "eth_renamed";
        await expect(
            rename(invalidInterfaceName, newInterfaceName),
        ).rejects.toThrowError(/Invalid old interface name: eth_invalid!/);
    });
    ...
    test("should reject with an error for a non-existent interface", async () => {
        ...
        const newInterfaceName = "eth_renamed";
        await expect(
            rename(nonExistentInterface, newInterfaceName),
        ).rejects.toThrowError(/Cannot find device/);
    });
...
describe("Set Mac", () => {
    ...
    test("should reject with an error for an invalid MAC address", async () => {
        ...
        await expect(setMac(interfaceName, invalidMacAddress)).rejects.toThrowError(
            /Invalid Mac Address/,
        );
    });
    ...
    test("should reject with an error for a non-existent interface", async () => {
        ...
        const newMacAddress = "00:11:22:33:44:55";
        await expect(
            setMac(nonExistentInterface, newMacAddress),
        ).rejects.toThrowError(/Cannot find device nonexistent/);
    });
});
