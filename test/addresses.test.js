...
describe("Add", () => {
    ...
    test("Should reject with an error for an invalid IP CIDR", async () => {
        ...
        await expect(add(interfaceName, invalidIpCidr)).rejects.toThrowError(
            /Invalid IP CIDR/,
        );
    });
    ...
    test("Should reject with an error for a non-existent interface", async () => {
        const nonExistentInterface = "nonexistent";
        const ipCidr = "172.16.17.1/32";
        await expect(add(nonExistentInterface, ipCidr)).rejects.toThrowError(
            /Cannot find device/,
        );
    });
    ...
    test("Should reject with an error if IP address already exists on interface", async () => {
        ...
        await expect(add(interfaceName, existingIpCidr)).rejects.toThrowError(
            /Error adding IP address: Error: ipv4: Address already assigned./,
        );
    });
});
...
describe("Remove", () => {
    test("Should resolve if IP address is successfully removed", async () => {
        ...
        await expect(
            remove(interfaceName, ipCidrToRemove),
    ...
    test("Should reject with an error for an invalid IP CIDR", async () => {
        const interfaceName = "eth_dummy_addr";
        const invalidIpCidr = "invalid_ip";
        await expect(remove(interfaceName, invalidIpCidr)).rejects.toThrowError(
            /Invalid IP CIDR/,
        );
    });
    ...
    test("Should reject with an error for a non-existent interface", async () => {
        ...
        const ipCidrToRemove = "172.16.17.1/32";
        await expect(
            remove(nonExistentInterface, ipCidrToRemove),
    ...
    test("Should reject with an error if IP address does not exist on interface", async () => {
        ...
        await expect(remove(interfaceName, nonExistingIpCidr)).rejects.toThrowError(
            /Error removing IP address: Error: ipv4: Address not found./,
        );
    });
});
...
describe("Flush", () => {
    ...
    test("Should reject with an error for a non-existent interface", async () => {
        const nonExistentInterface = "nonexistent";
        await expect(flush(nonExistentInterface)).rejects.toThrowError(
            /Error flushing IP addresses: Device/,
        );
    });
});
